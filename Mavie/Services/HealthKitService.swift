...
enum HealthKitService {
    ...
    /// Mavie symptoms that map to HK category types. Cravings (no good HK match)
    ...
    // MARK: - Import: Health → Mavie

    /// Read flow samples from Health and create or update Mavie CycleEntries.
    @discardableResult
    static func importFlowFromHealth(into context: ModelContext) async throws -> ImportResult {
        ...
        for sample in samples {
            ...
            guard let mavieFlow = mavieFlow(fromHKRawValue: sample.value) else { continue }
            if let entry = existing.first(where: { cal.isDate($0.date, inSameDayAs: day) }) {
                if entry.flow != mavieFlow {
                    entry.flow = mavieFlow
                    entry.updatedAt = .now
                    updated += 1
                }
            } else {
                let entry = CycleEntry(date: day, flow: mavieFlow)
    ...
    static func mavieFlow(fromHKRawValue rawValue: Int) -> FlowLevel? {
    ...
    private static func symptomSamples(from entry: CycleEntry) -> [HKCategorySample] {
        ...
        // Symptoms (severity defaults to mild — Mavie doesn't track severity per symptom)
