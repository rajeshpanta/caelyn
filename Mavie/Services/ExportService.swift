...
enum ExportService {
    ...
    static func generatePDF(entries: [CycleEntry], profile: UserProfile?, range: ExportRange, includeNotes: Bool) -> Data {
        ...
        let metadata: [String: Any] = [
            kCGPDFContextCreator as String: "Mavie",
            kCGPDFContextTitle as String:   "Mavie Cycle Report",
            kCGPDFContextAuthor as String:  "Mavie · Period Tracker"
        ]
    ...
    static func writeToTempFile(data: Data, format: ExportFormat, range: ExportRange) throws -> URL {
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        let basename = "Mavie-\(rangeFilenameFragment(range))-\(formatter.string(from: .now))"
